		...
		int th_num;
{
  ...
  POMP2_Parallel_fork(&opari2_region_1, pomp2_if, pomp2_num_threads, &pomp2_old_task, opari2_ctc_1 );
		#pragma omp parallel     private(closest, th_num) POMP2_DLIST_00001 firstprivate(pomp2_old_task) num_threads(pomp2_num_threads)
...
//==================================================================
	...
	if (end.tv_usec < 0) {  end.tv_usec += 1000000; end.tv_sec--; }
 	...
 	printf("\nTime to Kmeans is %ld seconds and %ld microseconds, %ld.%06ld\nChecksum value = %u\n", end.tv_sec, end.tv_usec, end, checksum);
